    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#F8F9FC" }}>
      ...
      {assignModalOpen && (() => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 anim-fade" onClick={() => setAssignModalOpen(null)}>
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="px-5 py-4 border-b border-slate-100 bg-slate-50">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-black text-slate-900 text-sm">Assign Instructor</h3>
                <button onClick={() => setAssignModalOpen(null)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="rounded-lg border border-slate-200 bg-white p-2.5 space-y-1.5">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-black" style={{ background: modalCfg.bg, color: modalCfg.color }}>
                    <span className="w-1.5 h-1.5 rounded-full" style={{ background: modalCfg.color }} /> {modalCfg.label}
                  </span>
                  <span className="text-[10px] font-bold text-slate-800 truncate">{modalSess.title}</span>
                </div>
                <div className="flex items-center gap-3 text-[10px] text-slate-500">
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {modalSess.dayOfWeek.slice(0,3)} {modalSess.startTime.split(" ")[1]}</span>
                  <span>Min: {modalSess.minSmeTier}</span>
                  <span>Topic: {modalSess.topic}</span>
                </div>
                <div className="flex flex-wrap gap-1">
                  {modalSess.requiredSkills.map(sk => (
                    <span key={sk} className="px-1.5 py-0.5 text-[9px] font-bold rounded-full bg-blue-50 text-blue-700 border border-blue-200">{sk}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="overflow-y-auto p-3 space-y-2">
              {schedule[assignModalOpen] && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 p-2.5 rounded-xl border border-red-200 bg-red-50 text-red-600 font-bold transition-all hover:bg-red-100 mb-1 shadow-sm text-xs"
                >
              ...
              {smes
                ...
                .map((item) => {
                  ...
                  <button
                    ...
                    className="w-full flex flex-col gap-2 p-3 rounded-xl border text-left transition-all hover:border-blue-400 hover:shadow-sm"
                    style={{
                      borderColor: isAssigned ? "#1E3A8A" : "#E2E8F0",
                      background: isAssigned ? "#EFF6FF" : ((!avail && !isAssigned) || hasCalendarConflict || dropped) ? "#F8F9FC" : "white",
                  ...
                  >
                    <div className="flex items-center gap-2.5 w-full">
                      <img src={sme.avatar} className="w-9 h-9 rounded-full object-cover flex-shrink-0 border border-slate-200" alt="" />
                      <div className="flex-1 min-w-0 text-xs">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-bold text-slate-900">{sme.name}</span>
